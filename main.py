...
def main() -> int:
    if "--list-objects" in sys.argv:
        ...
        print("Objects visible to UnrealCV (use one as UNREALCV_PAWN if pose is unavailable):")
        for n in names:
            print(f"  {n}")
        agent.disconnect()
        return 0
    ...
    host = os.environ.get("UNREALCV_HOST", "localhost")
    port = int(os.environ.get("UNREALCV_PORT", "9000"))
    pawn = os.environ.get("UNREALCV_PAWN", "BP_MyPlayer_Pawn_C")
    agent = UnrealAgent(host=host, port=port)
    ...
    try:
        run_pose_loop(agent, rate_hz=POSE_RATE_HZ, debug=debug)
