# Test-collection one-liner for outline trace 1 (world map). Append future flags to the @args array below
# instead of typing them out each time. Run from repo root:
#
#   pwsh -File scripts/test_collect1.ps1
#   # or in PowerShell:
#   .\scripts\test_collect1.ps1
#
# Pass extra flags through, e.g.:
#   .\scripts\test_collect1.ps1 --turn-threshold 10 --no-capture
...
$collectArgs = @(
    "--run-name",            "track1",
    "--map",                 "data/maps/outline_trace1_w.json",
    "--step",                "2",
    "--lookahead",           "2",
    ...
    "--num-runs",            "3",           # writes track1_r001, _r002, _r003
    "--debug"
