# Test-collection one-liner for outline trace 4 (world map). Same defaults as test_collect3.ps1.
# Append future flags to the @args array below instead of typing them out each time.
# Run from repo root:
#
#   pwsh -File scripts/test_collect.ps1
#   .\scripts\test_collect.ps1
#
# Pass extra flags through, e.g.:
#   .\scripts\test_collect.ps1 --turn-threshold 10 --no-capture
...
$collectArgs = @(
    ...
    "--min-finish-progress-ratio", "0.80",
    ...
    "--num-runs",            "3",           # writes track4_r001, _r002, _r003
    "--debug"
