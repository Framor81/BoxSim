# Test-collection one-liner for outline trace 2 (world map). Append future flags to the @args array below
# instead of typing them out each time. Run from repo root:
#
#   pwsh -File scripts/test_collect2.ps1
#   # or in PowerShell:
#   .\scripts\test_collect2.ps1
#
# Pass extra flags through, e.g.:
#   .\scripts\test_collect2.ps1 --turn-threshold 10 --no-capture
...
$collectArgs = @(
    "--run-name",            "track2",
    "--map",                 "data/maps/outline_trace2_w.json",
    "--step",                "2",
    "--lookahead",           "2",
    ...
    "--num-runs",            "3",           # writes track2_r001, _r002, _r003
    "--debug"
